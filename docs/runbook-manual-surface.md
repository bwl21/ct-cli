...
| Bereiche (departments) — creating/renaming one                                                      | The Bereich master data — the `cdb_bereich` permission scope dimension                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | [#108](https://github.com/eqrm/ct-cli/issues/108), [#109](https://github.com/eqrm/ct-cli/issues/109)                                            | **Moved here from "API gap" by #111.** `/departments` has no REST write verb (live-probed, eqrm prod CT 3.135.2, 2026-08-13) — but the admin UI creates Bereiche through `POST /index.php?q=churchdb/ajax` `func=saveMasterData` (captured 2026-08-13), so the old "can never be created" verdict was an artefact of auditing OpenAPI only. Until `ct` drives that endpoint: create/rename in **Master data admin → Bereiche**; `ct get departments` lists them. Grants scoped to one ARE declarable — `scope: [{ department: "<name-slug>" }]` resolves by name per host (#98). Renaming one in CT breaks every config reference to its old name, by design: the reference hard-errors instead of silently regranting elsewhere |
| Security levels — creating/reordering one                                                           | The `cc_securitylevel` master data (`GET /securitylevels`) — the scope dimension of `churchdb:+see persons` and friends                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | [#110](https://github.com/eqrm/ct-cli/issues/110) (ref form done; promotion open)                                                               | **REST CRUD exists** — live-probed 2026-08-14 (eqrm-dev, CT 3.135.2): create `POST /securitylevels/{id}`, update/reorder `PATCH /securitylevels/{id}` (`newid`, `forcereorder`), delete `DELETE /securitylevels/{id}`. So this is `ct`'s gap, not CT's: the id-bearing create does not fit the registry's "POST the collection path" contract. Until it is promoted, create/reorder in **Master data admin → Sicherheitslevel**; `ct get security-levels` lists them. Grants scoped to one are declarable by name — `scope: [{ securityLevel: "<name-slug>" }]` (#110) — and numeric `scope: [1, 2, 3]` still works, but check the ids per host before relying on them: they are auto-increment rows, not protocol constants     |
