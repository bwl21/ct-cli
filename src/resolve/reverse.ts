 ...
 * fetched resource carries (`campusId`, `groupTypeId`, `groupStatusId`) into the logical sugar the
 * DSL already accepts (`campus`/`groupType`/`status`), so an adopted snippet is portable and reads
 * like something a human would author — not a wall of instance-specific integers.
 *
 ...
 * (kind → path); kept explicit (three entries) rather than derived, with this comment as the sync note.
 */
const REVERSE_ID_FIELDS: Record<string, { catalog: string; sugar: string }> = {
  ...
  groupStatusId: { catalog: "/group/memberstatus", sugar: "status" },
};
...
export class ReverseResolver {
   ...
   * with NO match stays numeric and is named in `todos` so the emitter can flag it. Every other field
   * (and a `null` id — "no campus", omitted by the emitter) passes through in its original position.
   */
  async sugarFields(
